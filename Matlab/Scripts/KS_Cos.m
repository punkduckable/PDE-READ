% KS equation: D_t U = -1.0(D_x^2 U) - 1.0(D_x^4 U) - 1.0(D_x U)(U)
...
% Set up the spinop for the KS equation. For this problem:
%       L(u)    = -1.0(D_x^2 U) - 1.0(D_x^4 U) 
%       N(u)    = -1.0(U)(D_x U).
%       Init(x) = cos(pi*x/2.5)*(1 + sin(pi*x/5))
S           = spinop(Domain, T_span);
S.lin       = @(u)  -1.0*diff(u,2) - 1.0*diff(u,4);
S.nonlin    = @(u) -0.5*diff(u.^2);
S.init      = chebfun(@(x) cos(pi*x/2.5)*(1 + sin(pi*x/5)), Domain, 'vectorize');
